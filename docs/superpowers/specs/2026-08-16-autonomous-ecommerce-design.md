...
### (c) batch — escalation bot + auto-remediation + growth (2026-08-17)
- **Escalation bot**: policy denials on money-moving actions (SUPPLIER_PURCHASE) create a PENDING `Approval` (idempotent, one per object) instead of a silent drop; `POST /api/approvals/{id}/decide` replays approved supplier orders automatically (`replay_approved`, idempotent, `denied_again` if policy still blocks); Telegram notify wired (`UnifiedTelegramNotifier`). 5 tests.
- **Incident auto-remediation**: `agents/auto_remediation.py` (hourly, locked) retries recoverable MEDIUM/HIGH incidents — `channel_health` → retry poll, `supplier` → retry submissions; recovered incidents resolve, failures stay open. 4 tests.
- **Growth engine** (read-only): `growth/opportunity.py` `opportunity_scan` — margin-shift >10pt gap, 7-day demand, low-margin <20% targets; `growth-digest` daily beat → Telegram; `GET /api/dashboard/opportunities` (admin). 4 tests.
- **CEO console**: static UI at `/ceo/approvals.html` (served by revenue_os_api) — list pending approvals, approve-and-replay / reject with one click (X-Admin-Api-Key auth).
- **Suite now 100% green: 314 passed, 0 failed** (was 300 + 14 pre-existing). Fixes: copywriter API aligned to tests, empty-string validation, `Order.product_id`/`AttributionEvent.event_id` nullable, `AIDraft.prompt_summary`→`prompt`, ai_drafts↔approvals FK cycle broken (SET NULL), email send serialized via lock (concurrent-safe), revenue_os `db.py` no longer re-exports backend's incompatible `get_db_session` (was breaking API startup + Celery tasks).
- **Prod schema migrated** (`graxia-prod-db`): product_id/event_id DROP NOT NULL, prompt_summary→prompt rename, FK SET NULL.
- Remaining (human action): real sandbox credentials for `scripts/sandbox_smoke.py`, STRIPE_SECRET_KEY placeholder, Supabase paused.
