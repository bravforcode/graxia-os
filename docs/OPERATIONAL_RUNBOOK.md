...
Operational guide for the current Personal OS backend. This runbook is aligned with the code paths mounted by `backend/app/main.py` and the scripts in `scripts/ops/`.
...
```bash
bash scripts/ops/restore_database.sh --yes backups/backup_YYYYMMDD_HHMMSS.sql.gz
```
...
3. `bash -n scripts/ops/backup_database.sh`
4. `bash -n scripts/ops/restore_database.sh`
5. `bash -n scripts/ops/smoke_tests.sh`
6. `docker compose -f docker-compose.yml config > /dev/null`
7. `bash scripts/ops/smoke_tests.sh` against the running target stack
