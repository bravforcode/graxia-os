...
- **2026-08-07 (T1 #10-#12 + T2 #16-#22 batch, parallel subagents):** #10/#11/#12 CLOSED (28908bfc); #19 found Direction J collision (waiting on builder-dirj-3010 per user); #21 @0 dropped; #22 vault-sync hook fixed (hook-side); #16 note, #17/#18 deferred (design/user), #20 pending. Report: tier1_10_12_tier2_16_22_batch_20260807.md (af1ae28d)
...
| 10 | N-value for multiple-testing correction — claimed fixed (`get_reconciled_n_trials()` single source, N=1050) | **CLOSED 2026-08-07** (28908bfc): run_rydc_validation.py hardcoded n_trials=1001 replaced with get_reconciled_n_trials() (=1050). Single source confirmed at all 14+ DSR callers. Gaps noted: test sweeps only 1508; legacy run_dk_test clones lack correction |
| 11 | Sizing bug (100x units→lots) — mt5.py fixed; `risk/engine.py::_layer4()` and `position_sizer_v2.py` wiring/single-source-of-truth status | **VERIFIED SINGLE-SOURCE + WEBHOOK FIXED 2026-08-07** (4986a26f): all 3 points use core/contract_specs.py (mt5.py L276, _layer4 L539, position_sizer_v2 no-inline). webhook calculate_position_size JPY bug FIXED — was fabricated 100000/100=1000 + mixed USD-risk/JPY-price; now uses get_spec().tick_value conversion (USDJPY spec 100000/0.01/6.67). Regression test proves old=0.20 vs new=0.30 lots. Remaining: broker_adapter.py L200 deprecated (note only) |
| 12 | 5 other MT5 adapter methods with the same unguarded `_ensure_connected()` pattern as the fixed `submit_order()` (cancel_order, get_positions, close_position, get_account_info, set_stop_loss) | **CLOSED 2026-08-07** (28908bfc): WS-D entry guards verified; 3 mid-retry unguarded reconnects FIXED (cancel_order L410, close_position L581, set_stop_loss L679) + bonus get_price L630, update_trailing_stop L723. get_positions/get_account_info intentional propagate (8 callers wrap). ruff clean |
...
| 18 | `validation_stack` false-pass design decision (FAIL vs new UNKNOWN/ERRORED status for errored workstreams in gates.py) | **CLOSED 2026-08-07**: design decision already implemented in validation/pipeline/gates.py (GateStatus.ERRORED + error sentinels + FAIL>ERRORED>WARN>SKIP precedence, SP1 2026-08-04). Old governance/validation_stack.py DEPRECATED (resolves DEEP_AUDIT_FINDINGS L193-194 P1 false-pass bugs by deprecation, no fix). Repo-wide grep evidence committed |
| 19 | Trial-ledger ID collisions beyond the 2 already found — no systematic scan has been run across ALL ledgers (main, _b, _c, _d, _e, _g) at once | **COLLISION FOUND + IN PROGRESS 2026-08-07**: 16 checker problems. Root cause = Direction J UNREGISTERED using 3003-3009 inside B's 3001-3008 block (no _j in FAMILY_RANGES/TRIAL_ID_RANGES). Doc mismatches + trial_1030 no ledger entry. F26: writer lock held by builder-dirj-3010 (session renumbering J) — USER DECISION: wait, do not seize. Re-run checker after settle |
...
| 21 | 7 git stashes (@0-@12, actual count 7) — @3/@4 share identical message (needs diff before cleanup), @9 possibly TRADING_MODE-related | **PARTIAL 2026-08-07**: actual 21 stashes (not 7/13/20). @0 dropped (superseded, import types present). @3/@4 NOT identical (controller correction). @9 = coderabbit config (not TRADING_MODE). Rest = other sessions — untouched |
| 22 | Post-commit hook (Obsidian vault sync) — **status PARTIAL (2026-08-06):** fixed externally 13:26 2026-08-05; syntax valid (`bash -n` passes); behavioral evidence: 1 full sync completed 13:38 post-fix (`[SYNC] Done` + `Vault sync complete.`), but 5 entries "Starting vault sync..." 13:46-13:53 with NO completion lines → sync reliability needs follow-up verification (hanging/interrupted?) | **FIXED 2026-08-07 (hook-side, no commit)**: root cause MSYS PID recycle ($$ reused) + no age check → stale lock blocked sync forever. Hook now stores <pid> <epoch>, stale if PID dead OR >1800s. 10/10 tests. .git/hooks/post-commit + .bak |
