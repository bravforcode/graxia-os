...
|---|---|---|---|
| 1 | **Trial #2001/1022 date range** — Knowledge Dump claims "2005-2018, 12.99 ปี" but earlier investigation proved `data_range.end=2018` is pure metadata that doesn't gate the actual computation, and CSV data goes to 2026 | **NEVER RESOLVED — re-open this** | If the REJECT verdict that closed Direction A/B genuinely used only 13 years (no COVID/2022-inflation regime), it fails the plan's own 8-year regime-coverage requirement. This underpins the entire "directional prediction closed" decision. |
| 2 | Trial #4002 collision (main `trial_ledger.json` vs `hypothesis_registry_d.json`, same ID, different Direction) | **CLOSED 2026-08-05 no-action** (incident report; record never committed; checker passes 63+/0) | Root cause (parallel sessions writing main ledger) still live — hardener #19/#17 open |
| 3 | Direction D anomaly — main ledger cap raised 1022→1042 directly instead of opening `trial_ledger_d.json`, breaking the project's own Path-B precedent | **BLOCKING** — immediate pure-data fix done in C1 (lock_doc_path+cap fields corrected); deeper revert-vs-separate decision pending Sub-project B | Every subsequent Direction (E, G) had to work around this by explicitly avoiding the same mistake — the original instance is still unresolved; `auto_increment_trial.py` for ALL directions (incl Direction H) hits stale cap until resolved |
...
| # | Item | Status |
|---|---|---|
| 7 | Cost model 100x FX error / slippage≡spread — **direction of error** (over- vs under-stated) never actually determined | Assigned (F16), no confirmed completion in any later document |
| 8 | FOREX_EDGE_INVESTIGATION.md (6 trials: GBPUSD/USDJPY REJECT, 4 INCONCLUSIVE) — loader/runner script still **UNKNOWN** | Explicitly still open per F24 audit |
| 9 | Provenance backfill contamination re-check across the ~1050 historical trial history (NAS100 68% fake, EURUSD 56% fake pre-2005) | Explicitly deferred as "separate follow-up, not blocking" — never actually done |
