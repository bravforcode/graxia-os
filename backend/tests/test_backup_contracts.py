...
import pytest
from scripts.ops.backup_database import BackupManifest, DatabaseBackup, compute_sha256
from scripts.ops.restore_database import DatabaseRestore


import tempfile

def test_backup_manifest_preserves_plain_and_encrypted_checksums():
...
def test_database_backup_creates_age_encrypted_artifact_and_manifest(monkeypatch):
    with tempfile.TemporaryDirectory() as td:
        tmp_path = Path(td)
        monkeypatch.setattr("scripts.ops.backup_database.settings.BACKUP_DIR", str(tmp_path))
        monkeypatch.setattr(
            "scripts.ops.backup_database.settings.DATABASE_URL",
            "postgresql+asyncpg://user:password@postgres:5432/personal_os",
        )
        monkeypatch.setattr("scripts.ops.backup_database.settings.BACKUP_ENCRYPTION_PUBLIC_KEY", "age1test")
        monkeypatch.setattr("scripts.ops.backup_database.settings.BACKUP_BUCKET", "")
        monkeypatch.setattr("scripts.ops.backup_database.settings.APP_ENV", "test")
        ...
        monkeypatch.setattr("scripts.ops.backup_database.subprocess.run", fake_run)
...
def test_restore_refuses_encrypted_backup_without_private_key(monkeypatch):
    with tempfile.TemporaryDirectory() as td:
        tmp_path = Path(td)
        encrypted = tmp_path / "backup.dump.age"
        ...
        monkeypatch.setattr(
            "scripts.ops.restore_database.settings.BACKUP_ENCRYPTION_PRIVATE_KEY_FILE",
...
def test_restore_decrypts_and_verifies_plaintext_checksum(monkeypatch):
    with tempfile.TemporaryDirectory() as td:
        tmp_path = Path(td)
        encrypted = tmp_path / "backup.dump.age"
        encrypted.write_bytes(b"encrypted payload")
        ...
        monkeypatch.setattr(
            "scripts.ops.restore_database.settings.BACKUP_ENCRYPTION_PRIVATE_KEY_FILE", str(key_file)
        )
        monkeypatch.setattr("scripts.ops.restore_database.subprocess.run", fake_run)
