...
class TestCircuitBreakerFailFast:
    ...
    async def test_half_open_limits_calls(self):
        cb = CircuitBreaker("test", CircuitBreakerConfig(half_open_max_calls=2))
        cb._state = CircuitState.HALF_OPEN
...
class TestRealWorldScenarios:
    ...
    async def test_redis_network_timeout_scenario(self):
        ...
        cb = CircuitBreaker(
            "redis", CircuitBreakerConfig(failure_threshold=3, recovery_timeout=10.0)
        )
    ...
    async def test_openclaw_rate_limit_scenario(self):
        ...
        rate_limit_op = AsyncMock(side_effect=Exception("429 Too Many Requests"))
        for _ in range(5):
            with pytest.raises(Exception):
