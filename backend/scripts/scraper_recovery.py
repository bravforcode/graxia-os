...
from app.scrapers.smart_base import get_scraper_health, unmute_scraper
...
async def list_scraper_status():
    ...
    async with AsyncSessionLocal() as db:
        ...
        for health in health_records:
            ...
            last_error = (
                (health.last_error or "")[:27] + "..."
                if health.last_error and len(health.last_error) > 30
                else (health.last_error or "")
            )

            print(
                f"{health.source_name:<20} {muted:<8} {failures:<10} {success_rate:<10} {last_error:<30}"
            )
...
async def unmute_all_scrapers(notify: bool = True) -> int:
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(select(ScraperHealth).where(ScraperHealth.is_muted))
...
async def test_scraper(source_name: str) -> dict:
    ...
    return {"source": source_name, "found": True, **health}
  ...
  python -m scripts.scraper_recovery emergency
    ...
    async def run():
        if args.command == "list":
            ...
            confirm = input(
                "⚠️  This will unmute ALL scrapers and reset ALL failure counts. Continue? [y/N]: "
            )
            if confirm.lower() == "y":
