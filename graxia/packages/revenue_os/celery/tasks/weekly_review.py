...
async def weekly_review_with_db(db):
    """
    Weekly review implementation (db-injected — testable).
    ...
    async with acquire_automation_lock(db, "weekly_review", ttl_seconds=1800) as acquired:
        ...
        try:
            ...
            strategy_log = StrategyLog(
                ...
                summary=f"Week of {week_start.isoformat()}: {total_revenue / 100:.2f} THB revenue, {metrics['total_orders']} orders, {total_leads} leads",
...
async def _weekly_review_impl():
    async with get_db_session() as db:
        return await weekly_review_with_db(db)
