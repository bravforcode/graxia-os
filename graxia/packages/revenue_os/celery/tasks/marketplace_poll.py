...
poll_channel() is shared with the admin poll/backfill route.
"""
...
async def poll_channel(db: AsyncSession, channel: ChannelType,
                       since: Optional[str] = None) -> dict:
    """Poll ONE marketplace channel (source of truth import + reconcile).

    since: optional explicit cursor (admin backfill); default uses the
    channel config's last_import_at. The caller commits (config cursor is
    updated here but not committed)."""
    adapter_cls, reconcile_fn = ADAPTERS[channel]
    ...
    try:
        orders = await adapter.import_orders(since or config.get("last_import_at"))
        ...
        return {"fetched": len(orders), "imported": imported, "reconcile": reconcile}
...
async def marketplace_poll_with_db(db: AsyncSession) -> dict:
    async with acquire_automation_lock(db, LOCK_NAME, ttl_seconds=600) as acquired:
        ...
        for channel in ADAPTERS:
            results[channel.value] = await poll_channel(db, channel)
        await db.commit()
