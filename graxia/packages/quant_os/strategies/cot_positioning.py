"""
COT Positioning — CFTC Contrarian signal for XAUUSD.
...
it signals crowding — no marginal buyers (or sellers) left.

Contrarian logic:
- Extreme net-long → short bias (no marginal buyers)
- Extreme net-short → long bias (no marginal sellers)
...
def compute_cot_positioning_signals(
    ...
    2. Entry (contrarian):
         - z > +entry_z  →  SHORT (extreme net-long = crowding)
         - z < -entry_z  →  LONG  (extreme net-short = crowding)
    3. Exit when z reverts to ±exit_z.
    ...
    for i in range(len(zscore)):
        ...
        if in_position == 0:
            if z > config.entry_z:
                in_position = -1  # extreme long → short
                bars_in_position = 0
            elif z < -config.entry_z:
                in_position = 1   # extreme short → long
                bars_in_position = 0
        else:
            bars_in_position += 1
            # Exit conditions
