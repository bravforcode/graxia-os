"""
PGM Pairs Arbitrage — Platinum/Palladium structural dislocation.
...
1. South Africa controls 91% of platinum reserves but faces electricity
   crisis (60% cost increase since 2021) — supply pressure on platinum.
2. Russia supplies 40% of palladium under sanctions — Western premium.
...
def _engle_granger_cointegration(
    ...
    residual autocorrelation (simplified — no statsmodels dependency).
    """
    # Align
    ...
    # Simplified: |phi| > 0.05 → likely stationary (p < 0.05)
    if phi < -0.1:
...
def compute_pgm_pairs_signals(
    ...
    4. Exit when z reverts to ±exit_z.
    5. Stop-loss at ±stop_atr × ATR of spread.
