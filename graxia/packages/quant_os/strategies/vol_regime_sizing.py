"""
Vol Regime Sizing — Adaptive position sizing based on volatility regime.

Trial #1011 in the quant_os edge-search ledger.

ECONOMIC RATIONALE
==================
Volatility clusters — high-vol regimes persist and mean-revert. By sizing
...
This is NOT a directional signal — it's a position-sizing overlay that
...
def compute_vol_regime_sizing(
    ...
    4. Size multiplier:
         - LOW regime  → size_low (1.5x) — more aggressive
         - MED regime  → size_med (1.0x) — neutral
         - HIGH regime → size_high (0.5x) — defensive
    ...
    # 3) HAR forecast (optional — fit on available data)
