...
Signal:
  - RSI(14) < oversold_threshold → BUY (expect bounce back up)
  - RSI(14) > overbought_threshold → SELL (expect pullback down)
  - Optional EMA confirmation filter (price must be above/below EMA).

Exit: ATR-based SL/TP (identical to momentum_12m / donchian).

Reference: Welles Wilder (1978) — Relative Strength Index.
"""
    ...
    def generate_signal(
        ...
        **kwargs,
        ...
        if rsi < self.oversold:
            direction = 1  # BUY — oversold, expect bounce
        elif rsi > self.overbought:
            direction = -1  # SELL — overbought, expect pullback

        if direction == 0:
            return None

        # ── Optional EMA confirmation filter ──
        if self.ema_period > 0:
