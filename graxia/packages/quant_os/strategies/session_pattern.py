"""
Session Pattern (SP) â€” Volatility clustering by FX session.
...
- low_vol_session:  "asian"  (00:00-07:00 UTC)  â†’ mean reversion
- high_vol_session: "london" (07:00-12:00 UTC)  â†’ momentum
- high_vol_session: "ny"     (12:00-21:00 UTC)  â†’ momentum
...
def _atr(high: pd.Series, low: pd.Series, close: pd.Series, period: int) -> pd.Series:
    ...
    so the output is comparable across calls â€” the validation pipeline
...
def compute_sp_signals(
    ...
    5. Entry:
        - ASIAN bar AND deviation < -threshold_atr  â†’ LONG  (mean reversion)
        - ASIAN bar AND deviation > +threshold_atr  â†’ SHORT (mean reversion)
        - LONDON or NY bar AND deviation > +threshold_atr â†’ LONG  (momentum)
        - LONDON or NY bar AND deviation < -threshold_atr â†’ SHORT (momentum)
        - LATE_NY bar â†’ no signal (thin book).
