"""
Real-Yield Divergence Continuation (RYDC) Strategy â€” XAUUSD
...
- Residual z-score threshold: Â±1.5
- Hold period: 4 trading days fixed
- Stop-loss: 1.5 Ã— ATR(14)
- FOMC/CPI exclusion: 48h before/after events

Economic rationale:
- Gold carries no yield â†’ opportunity cost = real risk-free rate
- Real yields â†“ â†’ gold tailwind (falling opportunity cost)
- DXY â†“ â†’ gold tailwind (gold is USD-denominated)
...
class RYDCConfig:
    """Frozen configuration â€” cannot be changed after instantiation."""
    ...
    atr_multiplier: float = 1.5 # Stop-loss = 1.5 Ã— ATR
    event_exclusion_hours: int = 48  # Hours before/after FOMC/CPI to exclude


class RollingOLS:
    """Rolling OLS regression: Gold_ret = Î± + Î²1Â·DXY_ret + Î²2Â·Î”DFII10 + Îµ
