"""
Vol Risk Premium — Harvesting the implied-realized vol spread for XAUUSD.
...
volatility — the "volatility risk premium" (VRP). This premium exists
because options buyers overpay for tail-risk protection.

Strategy:
- When VRP is high (implied >> realized) → sell vol (short gold vol,
  or equivalently, mean-revert expecting vol to drop)
- When VRP is negative (realized >> implied) → buy vol (expect vol
  to stay elevated, trend-following)

In practice, we trade the *directional* implication:
- High VRP → gold likely to stay range-bound → mean-reversion signal
- Negative VRP → gold in vol expansion → trend-following signal
...
def compute_vol_risk_premium_signals(
    ...
    5. Regime detection:
         - VRP z > entry_z  →  HIGH premium regime (mean-reversion)
         - VRP z < -entry_z →  LOW premium / vol expansion (trend-following)
    6. Signal:
         - HIGH premium (VRP z > entry_z): expect vol contraction → LONG
           (mean-reversion: buy dips, sell rips)
         - LOW premium (VRP z < -entry_z): expect vol expansion → follow
    ...
    for i in range(len(df)):
        ...
        if in_position == 0:
            if z > config.entry_z:
                # High VRP → mean-reversion regime → long (expect vol contraction)
                in_position = 1
            elif z < -config.entry_z:
                # Low VRP → vol expansion → follow recent direction
