...
The hypothesis is therefore NOT "DXY moves → XAUUSD moves the same
...
# ---------------------------------------------------------------------------
# Frozen configuration (dataclass(frozen=True) — no runtime mutation)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Result container (plain NamedTuple — no relative imports)
# ---------------------------------------------------------------------------
...
def compute_cam_signals(
    ...
    3. Compute rolling 60d correlation between DXY returns and XAU returns
       ...
       relationship is currently negative — that's when gold is supposed
    ...
    5. Hold for `hold_days` trading days (or until the position is
       explicitly closed by the caller — this function returns -1/+1
