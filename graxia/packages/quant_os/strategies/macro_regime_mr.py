"""
Macro Regime Mean-Reversion (MRM) â€” regime-conditional signal.
...
the recent range â€” classical range-trading works. When DFII10 is
...
selection*:
    ...
    IF std(DFII10, 30d) / |mean(DFII10, 30d)|  <  regime_threshold:
        â†’ mean reversion  (price > 2*ATR above rolling mean â†’ short, etc.)
    ELSE:
        â†’ momentum       (price > 2*ATR above rolling mean â†’ long, etc.)
...
rejected on 2026-07-12) â€” RYDC tested cross-asset lead-lag on
...
(stopping_rule_2026_07_12.md Â§3.1) counts this as a new trial.
...
class Regime(str, Enum):
    ...
    STABLE = "stable"        # low CV of DFII10 â†’ mean reversion
    TRENDING = "trending"    # high CV of DFII10 â†’ momentum
...
def _atr(high: pd.Series, low: pd.Series, close: pd.Series, period: int) -> pd.Series:
    """Standard ATR â€” simple rolling mean (not Wilder-smoothed) so the
...
def compute_mrm_signals(
    ...
    1. Compute regime_score = std(DFII10, 30d) / |mean(DFII10, 30d)|
       (data through t-1). If regime_score < regime_threshold â†’ STABLE
       (mean reversion), else â†’ TRENDING (momentum).
    2. ATR(14) on the price series; rolling 20-bar mean of close.
    3. Deviation in ATR units: (close - rolling_mean_20) / ATR.
    4. Entry:
        - STABLE  AND deviation > +mr_threshold_atr â†’ SHORT  (revert down)
        - STABLE  AND deviation < -mr_threshold_atr â†’ LONG   (revert up)
        - TRENDING AND deviation > +mr_threshold_atr â†’ LONG   (trend up)
        - TRENDING AND deviation < -mr_threshold_atr â†’ SHORT  (trend down)
    ...
    # Align on common index (no forward fill â€” would leak)
    df = pd.concat(
    ...
    # STABLE â†’ mean reversion (short the upper extreme, long the lower extreme)
    signal[is_stable & (dev > config.mr_threshold_atr)] = -1
    signal[is_stable & (dev < -config.mr_threshold_atr)] = 1

    # TRENDING â†’ momentum (long the upper extreme, short the lower extreme)
