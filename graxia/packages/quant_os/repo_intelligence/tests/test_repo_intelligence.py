...
ADAPTERS_DIR = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "adapters"))
REGISTRY_DIR = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "registry"))
...
def test_adapter_has_normalize_output():
    ...
    for fname in ADAPTER_FILES:
        ...
        with open(path, encoding="utf-8") as fh:
            source = fh.read()
        assert "def normalize_output" in source, f"{fname} missing normalize_output function"
...
def test_adapter_has_validate_input():
    ...
    for fname in ADAPTER_FILES:
        ...
        with open(path, encoding="utf-8") as fh:
            source = fh.read()
        assert "def validate_input" in source, f"{fname} missing validate_input function"
...
def test_no_external_mt5_import():
    ...
    for fname in ADAPTER_FILES:
        ...
        with open(path, encoding="utf-8") as fh:
            source = fh.read()
        assert "MetaTrader5" not in source, f"{fname} references MetaTrader5 — forbidden in adapter"
...
def test_no_external_order_send():
    ...
    for fname in ADAPTER_FILES:
        ...
        with open(path, encoding="utf-8") as fh:
            source = fh.read()
        for fn in forbidden:
            assert fn not in source, f"{fname} references {fn} — forbidden in adapter"
...
def test_registry_has_required_fields():
    ...
    for entry in registry:
        for field in required:
            assert field in entry, f"repo {entry.get('repo_id', '???')} missing field '{field}'"
        perms = entry["permissions"]
        assert isinstance(perms, dict), f"repo {entry['repo_id']} permissions is not a dict"
...
def test_quarantined_repos_have_no_execution():
    ...
    registry = _load_registry()
    quarantined_map = {q["repo_id"]: q for q in _load_quarantined()}
    for entry in registry:
        if entry["repo_id"] in quarantined_map:
            perms = entry.get("permissions", {})
            assert (
                perms.get("read_only_reference") is False
            ), f"Quarantined {entry['repo_id']} has read_only_reference != false"
            assert (
                perms.get("isolated_execute") is False
            ), f"Quarantined {entry['repo_id']} has isolated_execute != false"
            assert (
                perms.get("mt5_execution_access") is False
    ...
    for fname in ADAPTER_FILES:
        ...
        for i, line in enumerate(lines, 1):
            ...
            for lib in external_libs:
                assert f"import {lib}" not in stripped, f"{fname}:{i} has module-level import of {lib}"
                assert f"from {lib}" not in stripped, f"{fname}:{i} has module-level from {lib} import"
