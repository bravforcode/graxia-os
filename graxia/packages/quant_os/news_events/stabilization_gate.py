...
class StabilizationGate:
    ...
    def __init__(self, event_store: EventStore, stabilization_minutes: int = 5, require_healthy_feed: bool = True):
        self._store = event_store
        self._stabilization_minutes = stabilization_minutes
        self._require_healthy_feed = require_healthy_feed

    def is_stabilized(
        self, at: datetime, currency: str, last_feed_healthy_at: datetime | None = None, spread_normal: bool = True
    ) -> GateResult:
        """Check if post-event stabilization period has elapsed and conditions are met."""
        import hashlib
        import json

        recent_high_events = self._store.query_at(as_of=at, currency=currency, min_importance="HIGH")

        relevant_events = [
            e
            for e in recent_high_events
