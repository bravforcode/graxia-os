...
class BrokerIdentityGuard:
    ...
    def validate(
        self, actual_server: str, actual_login: int, actual_mode: str, actual_currency: str
    ) -> tuple[bool, list[str]]:
        """Validate actual broker profile against expected."""
        self._violations = []

        if actual_server != self._expected.expected_server:
            self._violations.append(f"server_mismatch: expected={self._expected.expected_server}, got={actual_server}")

        if actual_mode != self._expected.account_mode:
            self._violations.append(f"account_mode_mismatch: expected={self._expected.account_mode}, got={actual_mode}")
        ...
        if actual_login != self._expected.account_login:
            self._violations.append(f"login_mismatch: expected={self._expected.account_login}, got={actual_login}")

        return len(self._violations) == 0, self._violations

    def compute_fingerprint(self, server: str, login: int, mode: str = "", currency: str = "") -> ProfileFingerprint:
