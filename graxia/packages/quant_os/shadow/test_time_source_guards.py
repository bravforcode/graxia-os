...
class TestRuleA:
    ...
    def test_tick_60s_late(self):
        now = datetime(2026, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        tick = datetime(2026, 1, 1, 12, 1, 0, tzinfo=timezone.utc)
        passed, diff = check_rule_a(tick, now)
        assert passed is True
        assert diff == 60000.0
    ...
    def test_tick_61s_late_fails(self):
        now = datetime(2026, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        tick = datetime(2026, 1, 1, 12, 1, 1, tzinfo=timezone.utc)
        passed, diff = check_rule_a(tick, now)
    ...
    def test_tick_3h_ahead_fails(self):
        ...
        now = datetime(2026, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        tick = datetime(2026, 1, 1, 15, 0, 0, tzinfo=timezone.utc)
        passed, diff = check_rule_a(tick, now)
        assert passed is False
        assert diff == 10800000.0  # 3 hours in ms

    def test_tick_none_fails(self):
        passed, diff = check_rule_a(None, datetime.now(timezone.utc))
        assert passed is False
...
class TestRuleB:
    def test_all_ticks_in_window(self):
        ...
        fr = datetime(2026, 6, 22, 12, 0, 0, tzinfo=timezone.utc)
        to = datetime(2026, 6, 22, 12, 1, 0, tzinfo=timezone.utc)
    ...
    def test_tick_outside_window(self):
        fr = datetime(2026, 6, 22, 12, 0, 0, tzinfo=timezone.utc)
        to = datetime(2026, 6, 22, 12, 0, 30, tzinfo=timezone.utc)
    ...
    def test_no_ticks_passes(self):
        passed, outside = check_rule_b([], datetime.now(timezone.utc), datetime.now(timezone.utc))
        assert passed is True
...
class TestRuleC:
    ...
    def test_no_bars_passes(self):
        passed, future = check_rule_c([], datetime.now(timezone.utc))
        assert passed is True
...
class TestRealDataAnomaly:
    def test_3h_offset_fails_rule_a(self):
        ...
        system_utc = datetime(2026, 6, 22, 16, 36, 28, tzinfo=timezone.utc)
        tick_utc = datetime(2026, 6, 22, 19, 36, 27, tzinfo=timezone.utc)
    ...
    def test_copy_ticks_range_stale(self):
        ...
        fr = datetime(2026, 6, 22, 16, 31, 28, tzinfo=timezone.utc)
        to = datetime(2026, 6, 22, 16, 36, 28, tzinfo=timezone.utc)
        passed, outside = check_rule_b(ticks, fr, to)
