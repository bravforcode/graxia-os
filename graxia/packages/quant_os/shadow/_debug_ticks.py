...
if tick:
    tick_dt = datetime.fromtimestamp(tick.time_msc / 1000, tz=timezone.utc)
...
if ticks is not None:
    ...
    if len(ticks) > 0:
        first = ticks[0]
        last = ticks[-1]
        print(f"  first: {int(first[0])} -> {datetime.fromtimestamp(int(first[0]), tz=timezone.utc).isoformat()}")
        print(f"  last: {int(last[0])} -> {datetime.fromtimestamp(int(last[0]), tz=timezone.utc).isoformat()}")
else:
    err = mt5.last_error()
