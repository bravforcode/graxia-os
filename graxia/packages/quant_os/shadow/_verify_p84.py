...
from graxia.packages.quant_os.shadow.canonical_tick_source import CanonicalTickPolicy, CanonicalTickSource
...
for cycle in range(5):
    ...
    ledger.append(
        {
            "cycle": cycle,
            "verdict": batch.verdict,
            "ticks": batch.deduplicated_tick_count,
            "hash": entry_hash,
            "prev": prev_hash,
        }
    )
    ...
    print(
        f"Cycle {cycle}: {batch.verdict} | ticks={batch.deduplicated_tick_count} | age={age:.0f}ms | bars={len(bars)}"
    )
