        ...
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            if isinstance(node, ast.ImportFrom) and node.module:
                for forbidden in FORBIDDEN_IMPORTS:
                    assert forbidden not in node.module, f"event_gate.py imports {forbidden}"
            if isinstance(node, ast.Import):
                for alias in node.names:
                    for forbidden in FORBIDDEN_IMPORTS:
                        assert forbidden not in alias.name, f"event_gate.py imports {forbidden}"
...
def test_market_health_no_execution_imports():
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            if isinstance(node, ast.ImportFrom) and node.module:
                for forbidden in FORBIDDEN_IMPORTS:
                    assert forbidden not in node.module, f"market_health.py imports {forbidden}"
            if isinstance(node, ast.Import):
                for alias in node.names:
                    for forbidden in FORBIDDEN_IMPORTS:
                        assert forbidden not in alias.name, f"market_health.py imports {forbidden}"
...
def test_event_risk_gate_no_execution_imports():
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            if isinstance(node, ast.ImportFrom) and node.module:
                for forbidden in FORBIDDEN_IMPORTS:
                    assert forbidden not in node.module, f"event_risk_gate.py imports {forbidden}"
            if isinstance(node, ast.Import):
                for alias in node.names:
                    for forbidden in FORBIDDEN_IMPORTS:
                        assert forbidden not in alias.name, f"event_risk_gate.py imports {forbidden}"
