...
from .broker_adapter import BrokerAdapter, BrokerManager, PaperBroker
    ...
    async def _submit_to_broker(
        ...
        if result.success:
            ...
            if result.status == OrderStatus.FILLED:
                ...
                # Pending fill
                state_machine.transition(result.status, f"Broker acknowledged: {result.broker_order_id}")
        else:
            state_machine.reject(result.error_message or "Broker error", "broker")
