...
.. deprecated::
    This module is deprecated. The canonical broker adapter hierarchy lives in
    ``execution/adapters/`` (``base.py``, ``mt5.py``, ``binance.py``,
    ``paper.py``, ``manager.py``). New code should import from there.
"""

import warnings

warnings.warn(
    "execution.broker_adapter is deprecated; use execution.adapters instead",
    DeprecationWarning,
    stacklevel=2,
)
...
class MT5BrokerAdapter(BrokerAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Fallback: try connecting to running terminal
            if self.mt5.initialize(timeout=timeout):
                self._connected = True
                return True

            # Last fallback: just path
            if self.mt5.initialize(path=path, timeout=timeout):
                self._connected = True
                return True
