...
class TestLeagueSystem:
    ...
    def test_consecutive_losses_bench(self):
        league = LeagueSystem()
        stats = {"test": {"trades": 5, "wins": 2, "losses": 3, "pnl": -100.0,
                          "total_win_pnl": 100.0, "total_loss_pnl": 200.0,
                          "active": True, "league_tier": "B"}}
        
        # First call initializes baseline (no counting)
        league.update(stats)
        
        # Simulate 3 more trades, all losses
        for i in range(3):
            stats["test"]["trades"] = 6 + i
            stats["test"]["losses"] = 4 + i
            stats["test"]["wins"] = 2
            league.update(stats)
