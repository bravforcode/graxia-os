...
class AggregatedSignal:
    ...
    consensus_entry: float | None = None
    consensus_sl: float | None = None
...
class GoldBotEngine:
    ...
    async def _fetch_data(self) -> dict:
        ...
        try:
            ...
            for tf in self.config.timeframes:
                bars = await self.broker.get_bars(self.config.symbol, tf, 200)
                if bars:
                    data[tf] = {
    ...
    async def _execute_signal(self, signal: AggregatedSignal):
        ...
        if not self.broker:
            print(
                f"  [SIMULATED] {signal.direction.value} XAUUSD "
        ...
        try:
            ...
            dynamic_sl, dynamic_tp = self._calculate_dynamic_sl_tp(entry_price, signal.direction)
            ...
            if result.success:
                ...
                print("\n  [BUY] TRADE EXECUTED" if side == OrderSide.BUY else "\n  [SELL] TRADE EXECUTED")
        ...
        try:
            ...
            for trade in self.open_trades:
                ...
                if (
                    ...
                    or trade.direction == SignalDirection.SELL
                    and current_price >= trade.stop_loss
                ):
    ...
    async def _close_trade(self, trade: TradeRecord, exit_price: float, reason: str):
        ...
        for strat_name, _score in trade.strategy_scores.items():
    ...
    async def _print_summary(self):
        ...
        for name, stats in sorted(self.strategy_stats.items(), key=lambda x: x[1]["pnl"], reverse=True):
            if stats["signals"] > 0:
                win_rate = (stats["wins"] / stats["trades"] * 100) if stats["trades"] > 0 else 0
                print(
                    f"  {name:<20} {stats['signals']:<10} {stats['trades']:<10} "
                    f"{win_rate:<10.1f} ${stats['pnl']:+,.2f}"
                )

        print("\n  League Status:")
        for tier in ["S", "A", "B", "C", "BENCH"]:
            members = [n for n, s in self.strategy_stats.items() if s.get("league_tier") == tier]
            if members:
