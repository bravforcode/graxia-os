...
class Order(Base):
    ...
    time_in_force: Mapped[TimeInForce] = mapped_column(
        SAEnum(TimeInForce), default=TimeInForce.DAY
    )

    strategy_id: Mapped[str] = mapped_column(String(64), nullable=False)
    signal_id: Mapped[Optional[UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("quant_signals.id")
    )
    # Kept as plain UUID to avoid a circular FK with quant_risk_checks.
    risk_check_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True))
    ...
    status: Mapped[OrderStatus] = mapped_column(
        SAEnum(OrderStatus), default=OrderStatus.CREATED
    )
    ...
    fills: Mapped[List["Fill"]] = relationship(
        "Fill", back_populates="order", lazy="dynamic"
    )
    signal: Mapped[Optional["Signal"]] = relationship(
        "Signal", back_populates="orders"
    )
...
class OrderStateHistory(Base):
    ...
    order_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("quant_orders.id"), nullable=False
    )
...
class Position(Base):
    ...
    position_type: Mapped[PositionType] = mapped_column(
        SAEnum(PositionType), nullable=False
    )
    ...
    entry_order_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("quant_orders.id")
    )
    exit_order_id: Mapped[Optional[UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("quant_orders.id")
    )
...
class Signal(Base):
    ...
    strength: Mapped[Optional[str]] = mapped_column(String(16))

    regime: Mapped[Optional[RegimeType]] = mapped_column(SAEnum(RegimeType))
    indicator_values: Mapped[Optional[dict]] = mapped_column(JSONB)
...
class RiskCheck(Base):
    ...
    result: Mapped[RiskCheckResult] = mapped_column(
        SAEnum(RiskCheckResult), nullable=False
    )
...
class Backtest(Base):
    ...
    initial_capital: Mapped[Decimal] = mapped_column(
        Numeric(18, 2), default=Decimal("10000")
    )
    ...
    slippage_used: Mapped[Decimal] = mapped_column(
        Numeric(8, 4), default=Decimal("0.5")
    )
    commission_used: Mapped[Decimal] = mapped_column(
        Numeric(8, 4), default=Decimal("3.5")
    )
...
class BacktestTrade(Base):
    ...
    backtest_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("quant_backtests.id"), nullable=False
    )
...
class PortfolioSnapshot(Base):
    ...
    portfolio_exposure_pct: Mapped[Decimal] = mapped_column(
...
class KillSwitchEvent(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    switch_type: Mapped[KillSwitchType] = mapped_column(
        SAEnum(KillSwitchType), nullable=False
    )
...
class StrategyRegistry(Base):
    ...
    status: Mapped[StrategyStatus] = mapped_column(
        SAEnum(StrategyStatus), default=StrategyStatus.HYPOTHESIS
    )
...
class MLModel(Base):
    ...
    status: Mapped[ModelStatus] = mapped_column(
        SAEnum(ModelStatus), default=ModelStatus.RESEARCH
    )
...
class AuditLog(Base):
    ...
    result: Mapped[Optional[str]] = mapped_column(String(16))
    context: Mapped[Optional[dict]] = mapped_column(JSONB)
...
class DataQualityRun(Base):
    ...
    symbol: Mapped[Optional[str]] = mapped_column(String(16))
    check_name: Mapped[DataQualityCheck] = mapped_column(
        SAEnum(DataQualityCheck), nullable=False
    )
...
class ReconciliationLog(Base):
    ...
    status: Mapped[ReconciliationStatus] = mapped_column(
        SAEnum(ReconciliationStatus), nullable=False
    )
    severity: Mapped[Optional[str]] = mapped_column(String(8))
