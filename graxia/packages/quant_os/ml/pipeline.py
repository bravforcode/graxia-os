...
class FeatureEngineer:
    ...
    def generate_features(
        ...
        features["log_return_1"] = (df["close"] / df["close"].shift(1)).apply(lambda x: __import__('math').log(x) if x > 0 else 0)
...
class MLTrainer:
    ...
    def train(
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=test_ratio, shuffle=False  # Time series - don't shuffle
        )
        ...
        if hasattr(model, "feature_importances_"):
            ...
            for name, imp in zip(feature_set.feature_names, importances):
    ...
    def predict(
        self, model_path: str, features: Dict[str, float]
    ) -> Tuple[int, float]:
        """
