"""
Minimal ticker extraction audit — 20 headlines, direct LLM calls.
"""
...
# (headline, expected_ticker_or_None, category)
TESTS = [
...
def run_llm(headlines):
    prompt = (
        "Extract the primary ticker from each headline.\n"
        'Return JSON: [{"s":"headline","k":"TICKER"}]\n'
        "If no specific ticker, use k=null.\n"
        "Standard tickers: AAPL, MSFT, NVDA, TSLA, AMZN, META, GOOGL, BTC-USD, ETH-USD, "
        "CL=F, GC=F, ^GSPC, ^DJI, ^IXIC.\n\n"
        "Headlines:\n"
    )
...
def main():
    print("=" * 60)
    print("TICKER EXTRACTION AUDIT (20 headlines)")
    print("=" * 60)
    ...
    for r in all_results:
        if r["mode"] not in ("valid", "correct_none"):
            print(f"  [{r['mode']}] expected={r['expected']} got={r['extracted']}")
            print(f"    \"{r['headline']}\"")

    # Save
    out = os.path.join(os.path.dirname(__file__), "..", "state", "ticker_audit_results.json")
