...
def run_llm(headlines):
    """Run LLM on a batch of headlines. Returns raw output."""
...
def parse_output(raw):
    """Parse LLM JSON output into list of (headline, ticker)."""
    # Try JSON parse
    try:
        clean = raw.strip()
        if clean.startswith("```"):
...
def main():
    ...
    for i in range(0, len(TESTS), 5):
        ...
        log.append(f"Raw output length: {len(raw)} chars")
        log.append(f"Raw preview: {raw[:200]}...")

        parsed = parse_output(raw)
        log.append(f"Parsed {len(parsed)} items from LLM output")
    ...
    # Write log
    out_path = os.path.join(STATE, "ticker_audit_results.txt")
