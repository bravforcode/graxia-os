...
def _make_bar_events(n=30, trend=0.001, bar_range_pct=0.0005):
    ...
    for i in range(n):
        ...
        bars.append(BarEvent(
            symbol="XAUUSD", high=c + r, low=c - r,
            open=c - r * 0.5, close=c + r * 0.5, volume=1000,
        ))
    return bars
...
def _make_signal_event(confidence=0.8, source="technical_analyst"):
    return SignalEvent(
        symbol="XAUUSD", signal_type=SignalType.BUY, confidence=confidence,
        entry_price=2400.0, stop_loss=2390.0, take_profit=2420.0, source=source,
    )


def _make_risk_signal(conviction=0.8, entry=2400.0, sl=2390.0, tp=2420.0):
    return RiskSignal(
        symbol="XAUUSD", conviction=conviction,
...
@pytest.fixture
def healthy_account():
    return AccountState(equity=100000, balance=100000, daily_pnl=0, weekly_pnl=0, max_drawdown_pct=0.02, margin_level_pct=500)
...
class TestE2EHappyPath:
    ...
    def test_ml_to_technical_to_risk_to_execution(
        self, technical_analyst, portfolio_manager, risk_auditor, risk_engine,
        healthy_account, clean_portfolio, mock_broker_adapter, tmp_path,
    ):
        ...
        if tech_signal.entry_price and tech_signal.stop_loss:
            ...
            tech_signal = SignalEvent(
                symbol=tech_signal.symbol, signal_type=tech_signal.signal_type,
                confidence=tech_signal.confidence, entry_price=float(tech_signal.entry_price),
                stop_loss=float(tech_signal.stop_loss), take_profit=round(min_tp, 2),
        ...
        sentiment = SignalEvent(
            symbol="XAUUSD", signal_type=SignalType.HOLD, confidence=1.0,
            source="sentiment_agent", metadata={"position_multiplier": 1.0},
        )
        ...
        risk_signal = _make_risk_signal(
            conviction=final_signal.confidence, entry=final_signal.entry_price,
            sl=final_signal.stop_loss, tp=final_signal.take_profit,
        )
        verdict = risk_engine.evaluate(
            signal=risk_signal, account=healthy_account, portfolio=clean_portfolio,
...
class TestE2ERiskRejection:
    ...
    def test_daily_loss_limit_blocks_trade(self, risk_engine, clean_portfolio):
        ...
        verdict = risk_engine.evaluate(signal=signal, account=account, portfolio=clean_portfolio, realized_vol=0.15, regime=RegimeType.RANGE_BOUND)
        assert verdict.approved is False
    ...
    def test_total_exposure_blocks_trade(self, risk_engine, healthy_account):
        ...
        signal = _make_risk_signal(conviction=0.8)
        verdict = risk_engine.evaluate(signal=signal, account=healthy_account, portfolio=portfolio, realized_vol=0.15, regime=RegimeType.RANGE_BOUND)
        assert verdict.approved is False
...
class TestE2ECircuitBreaker:
    ...
    def test_kill_switch_blocks_all_signals(self, kill_switch, circuit_breaker):
        ...
        verdict = engine.evaluate(signal=signal, account=account, portfolio=portfolio, realized_vol=0.15, regime=RegimeType.RANGE_BOUND)
        assert verdict.approved is False
...
class TestE2ESizing:
    ...
    def test_sizing_with_high_vol_reduces_quantity(self, risk_engine, healthy_account, clean_portfolio):
        signal = _make_risk_signal()
        verdict_low = risk_engine.evaluate(signal=signal, account=healthy_account, portfolio=clean_portfolio, realized_vol=0.5, regime=RegimeType.RANGE_BOUND)
        verdict_high = risk_engine.evaluate(signal=signal, account=healthy_account, portfolio=clean_portfolio, realized_vol=0.1, regime=RegimeType.RANGE_BOUND)
