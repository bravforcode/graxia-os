...
def _make_request(body: bytes | str | dict | None, headers: dict | None = None) -> MagicMock:
    ...
    req.app.state.signal_gateway.ingest = AsyncMock(return_value=MagicMock(
        signal_id="sig-001", symbol="EURUSD", side=MagicMock(value="BUY"),
    ))
    return req
...
class TestAdminChaos:
    ...
    async def test_change_mode_invalid_mode(self):
        from graxia.packages.quant_os.api.admin import change_trading_mode, ModeChangeRequest
        with pytest.raises(Exception):
    ...
    async def test_update_strategy_empty_id(self):
        from graxia.packages.quant_os.api.admin import update_strategy, StrategyUpdateRequest
...
class TestDbChaos:
    ...
    async def test_get_db_timeout(self):
        with patch("graxia.packages.quant_os.api.db._get_engine") as m:
            m.side_effect = asyncio.TimeoutError("Connection timed out")
            from graxia.packages.quant_os.api.db import get_db
...
class TestOrdersChaos:
    ...
    async def test_create_order_not_implemented(self):
        from graxia.packages.quant_os.api.orders import create_order, OrderCreateRequest
        mock_db = AsyncMock()
    ...
    async def test_list_orders_with_filters(self):
        from graxia.packages.quant_os.api.orders import list_orders, OrderStatus
        mock_db = MagicMock()
        ...
        result = await list_orders(status=OrderStatus.FILLED, symbol="EURUSD", strategy_id="mtm", limit=10, offset=0, db=mock_db)
        assert result.total == 0
...
class TestRiskChaos:
    ...
    async def test_kill_switch_invalid_action(self):
        ...
        with patch("graxia.packages.quant_os.api.risk.get_config", return_value=_mock_config()):
            with pytest.raises(Exception):
                await kill_switch_action(
                    request=KillSwitchActionRequest(action="INVALID_ACTION", reason="test")
                )
...
class TestWebhookChaos:
    ...
    async def test_manual_signal_wrong_api_key(self):
        from graxia.packages.quant_os.api.webhook import manual_signal, TradingViewPayload
        with patch("graxia.packages.quant_os.api.webhook.get_config", return_value=_mock_config()):
            payload = TradingViewPayload(
                action="buy", symbol="EURUSD", price=1.085,
                sl=1.082, tp=1.091,
            )
            with pytest.raises(Exception):
    ...
    def test_calculate_position_size_returns_decimal(self):
        from graxia.packages.quant_os.api.webhook import calculate_position_size, TradingViewPayload
        with patch("graxia.packages.quant_os.api.webhook.get_config", return_value=_mock_config()):
            payload = TradingViewPayload(
                action="buy", symbol="EURUSD", price=1.085,
                sl=1.082, tp=1.091,
...
class TestWebhookReceiverChaos:
    ...
    def _gw_with_signal(self, signal_id="s1", symbol="XAUUSD"):
        gw = MagicMock()
        gw.ingest = AsyncMock(return_value=MagicMock(
            signal_id=signal_id, symbol=symbol, side=MagicMock(value="BUY"),
        ))
        return gw
