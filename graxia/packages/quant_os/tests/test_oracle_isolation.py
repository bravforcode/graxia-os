...
def test_no_oracle_imports_in_canonical():
    """Canonical source must not import oracle packages."""
    for py_file in CANONICAL_DIR.rglob("*.py"):
        if "adapters" in str(py_file):
            continue
                ...
                for alias in node.names:
                    for pkg in ORACLE_PACKAGES:
                        if pkg in alias.name.lower():
                            assert False, f"Oracle import in {py_file.name}:L{node.lineno}: {alias.name}"
            if isinstance(node, ast.ImportFrom):
                if node.module:
                    for pkg in ORACLE_PACKAGES:
                        if pkg in node.module.lower():
                            assert False, f"Oracle import in {py_file.name}:L{node.lineno}: from {node.module}"
