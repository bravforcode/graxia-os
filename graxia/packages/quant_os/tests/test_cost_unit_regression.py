...
def _check_cost_sanity(spread_cost: float, slippage: float, label: str = ""):
    ...
    result = compute_fold_pnl(returns, preds, confs,
    ...
    msg = (f"[{label}] cost/trade ${cost_per_trade:.6f} "
    ...
    assert abs(cost_per_trade - expected) < 0.001, (
        f"COST VALUE BUG: {msg}"
    )
    return result
...
def test_cost_sanity_zero_trades():
    ...
    result = compute_fold_pnl(
        ...
        spread_cost=0.0001, slippage_p90=0.00005,
        min_confidence=0.5,
    )
...
def test_trade_pnl_uses_per_trade_price():
    ...
    result = compute_trade_pnl(
        ...
        lot_mult=1.0, close_prices=df["close"].values,
    )
...
def test_trade_pnl_broker_realistic_bounds():
    ...
    assert 0.05 <= cost_per_trade <= 2.0, (
        f"Cost ${cost_per_trade:.4f}/trade outside broker-realistic range $0.05-$2.00"
    )
...
def test_trade_pnl_cost_scales_with_price():
    ...
    assert 1.6 < ratio < 1.8, (
        f"Cost ratio {ratio:.2f} should be ~1.7 (4000/2350)"
    )
