...
class Order(Base):
    ...
    risk_check_id: Mapped[UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("quant_risk_checks.id"))
    compliance_check_id: Mapped[UUID | None] = mapped_column(UUID(as_uuid=True))
    approved_by: Mapped[str | None] = mapped_column(String(64))  # Human approver for MICRO
...
class Signal(Base):
    ...
    # Confidence metrics
    confidence: Mapped[Decimal | None] = mapped_column(Numeric(5, 4))  # 0.0 to 1.0
    strength: Mapped[str | None] = mapped_column(String(16))  # weak, medium, strong

    # Market context
    regime: Mapped[RegimeType | None] = mapped_column(SAEnum(RegimeType))
    indicator_values: Mapped[dict | None] = mapped_column(JSONB)  # ADX, RSI, ATR, etc.

    # Source
    raw_payload: Mapped[dict | None] = mapped_column(JSONB)  # Original webhook payload
    ...
    order_id: Mapped[UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("quant_orders.id"))
...
class Backtest(Base):
    ...
    equity_curve: Mapped[list | None] = mapped_column(JSONB)  # List of {date, equity}
...
class AuditLog(Base):
    ...
    result: Mapped[str | None] = mapped_column(String(16))  # SUCCESS, FAILURE, BLOCKED
    context: Mapped[dict | None] = mapped_column(JSONB)
    prev_hash: Mapped[str | None] = mapped_column(String(64))  # Hash chain for tamper evidence
    occurred_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class ReconciliationLog(Base):
    ...
    severity: Mapped[str | None] = mapped_column(String(8))  # NULL, SMALL, MEDIUM, LARGE
