"""
Pooled Multi-Asset Donchian+RSI Test — Driscoll-Kraay Cluster-Robust Inference
================================================================================
Same universe/method as pooled_donchian_test.py, swapping DonchianBreakout for
DonchianRSI (breakout + RSI momentum filter).

Tests period=20 and period=55 (matching the plain Donchian variants).

Method:
  1. Run BacktestEngine with DonchianRSI on each of 8 assets
...
# Period variants (matching plain Donchian test)
...
def run_engine_for_asset(symbol: str, period: int) -> dict:
    """Run BacktestEngine with DonchianRSI on one asset. Returns results dict."""
    from graxia.packages.quant_os.backtest.engine import BacktestConfig, BacktestEngine
    from graxia.packages.quant_os.strategies.donchian_rsi import DonchianRSI
    ...
    strategy = DonchianRSI(
        ...
        rsi_overbought=FIXED_PARAMS["rsi_overbought"],
        rsi_oversold=FIXED_PARAMS["rsi_oversold"],
...
def run_one_period(period: int) -> dict:
    print("=" * 70)
    print(f"  POOLED MULTI-ASSET DONCHIAN+RSI({period}) TEST")
    print("  Driscoll-Kraay Cluster-Robust Inference")
    print(
        f"  Params: period={period}, atr_sl={FIXED_PARAMS['atr_sl_mult']}, "
        f"atr_tp={FIXED_PARAMS['atr_tp_mult']}, rsi_ob={FIXED_PARAMS['rsi_overbought']}, "
        f"rsi_os={FIXED_PARAMS['rsi_oversold']}, vol_filter_pctile={FIXED_PARAMS['vol_filter_pctile']}"
    )
    print("=" * 70)
...
def main():
    ...
    for period in PERIOD_VARIANTS:
        reports[f"donchian_rsi_{period}"] = run_one_period(period)
        print("\n")
    ...
    report_path = ROOT / "reports" / "pooled_donchian_rsi_results.json"
