...
for i in range(5):
    rec = LedgerRecord(
        trade_id=f"t-{i}", order_id=f"o-{i}", symbol="XAUUSD", side="BUY",
        entry_price="2000", exit_price="2010", volume="0.1", pnl="10",
        fees="3.5", spread_cost="2", slippage_cost="1",
        entry_time="2025-01-01T00:00:00", exit_time="2025-01-02T00:00:00",
        close_reason="TAKE_PROFIT", strategy_id="test",
        contract_snapshot_id="v1", risk_policy_version="DEFAULT",
        dataset_manifest_id="d1", cost_scenario="BASE", git_commit="test",
    )
    chain.append(rec)
