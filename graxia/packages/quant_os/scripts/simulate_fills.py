        ...
        for side in sides:
            ...
            for lat_ms in latencies_ms:
                ...
                slippage_pts = (
                    ...
                    if side == "buy"
                    else (decision_price_fast - fill_price) / point_val
                )
...
def main():
    ...
    parser.add_argument("--symbol", type=str, default="XAUUSD", help="Symbol to simulate fills for")
    parser.add_argument(
        "--tick-dir", type=str, default=os.path.join("artifacts", "mega_data", "ticks"), help="Directory with tick data"
    )
    parser.add_argument("--freq", type=str, default="1min", help="Decision frequency (bar close interval)")
    parser.add_argument(
        "--latencies", type=str, default="50,150,300,500", help="Comma-separated latency assumptions in ms"
    )
    parser.add_argument("--sides", type=str, default="buy,sell", help="Order sides to simulate")
    parser.add_argument(
        "--output", type=str, default=os.path.join("artifacts", "fill_samples"), help="Output directory"
    )
    ...
    for lat in latencies_ms:
        ...
        if len(subset) > 0:
            ...
            print(
                f"  {lat}ms: n={len(sl)}, P50={sl.quantile(0.5):.2f}, P90={sl.quantile(0.9):.2f}, "
    ...
    for bucket_col in ["session", "vol_regime", "spread_bucket"]:
        ...
        for bucket in sorted(df[bucket_col].unique()):
            ...
            if len(sl) > 0:
                print(
                    f"    {bucket}: n={len(sl)}, P50={sl.quantile(0.5):.2f}, "
