...
Training strategy:
  1. Walk-forward validation (80/20 split, no look-ahead)
  2. Feature selection via mutual information (top 50)
  3. Optuna hyperparameter tuning (100 trials)
  4. Train 3 models + ensemble soft voting
"""
...
def load_data() -> pd.DataFrame:
    ...
    # Create targets from close prices
    if "close" not in df.columns:
        raise ValueError("No 'close' column found — cannot create targets")

    # next_bar_return: return of the next bar
    df["next_bar_return"] = df["close"].shift(-1) / df["close"] - 1

    # is_long: 1 if next bar goes up (next_bar_return > 0), 0 if down
    df["is_long"] = (df["next_bar_return"] > 0).astype(int)

    # Drop last row (no target)
    df = df.dropna(subset=["next_bar_return", "is_long"])

    log(f"  After target creation: {df.shape[0]} rows")
    log(f"  is_long distribution: {df['is_long'].value_counts().to_dict()}")
    log(f"  Mean next_bar_return: {df['next_bar_return'].mean():.6f}")
...
def train_catboost(X_train, y_train, X_test, y_test, feature_names):
    """Train CatBoost with sensible defaults."""
...
def compute_trading_metrics(y_true, y_pred, next_bar_returns):
    ...
    # Sharpe ratio (annualized, assuming 15min bars ≈ 35040/year)
    # ponytail: minimum 30 returns needed for reliable std estimate
    if len(returns) >= 30 and np.std(returns) > 0:
        ...
        # Small sample: annualize but mark as unreliable
...
def walk_forward_evaluate(X, y, model, n_folds=5, embargo=12):
...
def main():
    ...
    log("=" * 70)
    log("MEGA MODEL TRAINING — XGBoost + LightGBM + CatBoost Ensemble")
    log("=" * 70)
    ...
    y_all = df["is_long"].values.astype(int)
    returns_all = df["next_bar_return"].values
    ...
    log(f"\n{'='*70}")
    log(f"OPTUNA TUNING — {N_TRIALS} trials (XGBoost)")
    log(f"{'='*70}")
    ...
    objective_fn = make_objective(X_train_sel, y_train_full, selected_features)
    ...
    try:
        ...
        log(f"\n  Deflated Sharpe: {ds_result.deflated_sharpe:.4f} (pass={ds_result.passes_threshold})")
        log(f"    Multiple-testing adjustment: {ds_result.multiple_testing_adjustment:.4f}")
        log(f"    Probability of false positive: {ds_result.probability_alpha:.4f}")
    except Exception as e:
        log(f"  [WARN] Deflated Sharpe failed: {e}")

    # 8. Walk-forward cross-validation
    log(f"\n{'='*70}")
    log("WALK-FORWARD CROSS-VALIDATION (5 folds)")
    log(f"{'='*70}")

    wf_accs = walk_forward_evaluate(X_train_sel, y_train_full, xgb_model, n_folds=5)
    ...
    # Ensemble
    ens_path = OUT_DIR / f"mega_ensemble_{TIMESTAMP}.pkl"
    ensemble_dict = {
        "xgboost": xgb_model,
        "lightgbm": lgb_model,
        "catboost": cb_model,
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("=" * 70 + "\n")
        f.write("MEGA MODEL TRAINING REPORT\n")
        ...
        f.write(f"  is_long distribution: {df['is_long'].value_counts().to_dict()}\n\n")
    ...
    # Also save to ML models dir for paper_trade_bot compatibility
