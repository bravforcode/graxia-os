...
def _read_partitioned_dir(dir_path: str, symbols: list[str]) -> dict[str, pd.DataFrame]:
...
def compute_features(ohlc: pd.DataFrame) -> pd.DataFrame:
    ...
    df["spread_zscore"] = (df["spread_pips"] - df["spread_pips"].rolling(50).mean()) / (
...
def compute_session_features(ohlc: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    df["session_asian"] = ((time_of_day >= ASIAN_OPEN.hour) & (time_of_day < ASIAN_CLOSE.hour)).astype(int)
    df["session_european"] = ((time_of_day >= EURO_OPEN.hour) & (time_of_day < EURO_CLOSE.hour)).astype(int)
    df["session_us"] = ((time_of_day >= US_OPEN.hour) & (time_of_day < US_CLOSE.hour)).astype(int)

    df["session_overlap_ae"] = (
        (time_of_day >= max(ASIAN_OPEN.hour, EURO_OPEN.hour)) & (time_of_day < min(ASIAN_CLOSE.hour, EURO_CLOSE.hour))
    ).astype(int)
    df["session_overlap_eu"] = (
        (time_of_day >= max(EURO_OPEN.hour, US_OPEN.hour)) & (time_of_day < min(EURO_CLOSE.hour, US_CLOSE.hour))
    ).astype(int)
    ...
    for name, open_hour in session_opens.items():
        df[f"minutes_since_{name}_open"] = np.maximum(0, (time_of_day - open_hour) * 60)

    session_avg_atr = df.groupby(4 * df["session_asian"] + 2 * df["session_european"] + 1 * df["session_us"])[
        "atr_14"
    ].transform("mean")
...
def compute_regime_features(ohlc: pd.DataFrame) -> pd.DataFrame:
    ...
    plus_di_14 = 100 * pd.Series(plus_dm, index=df.index).rolling(14).mean() / (atr_14 + 1e-10)
    minus_di_14 = 100 * pd.Series(minus_dm, index=df.index).rolling(14).mean() / (atr_14 + 1e-10)
...
def compute_microstructure_features(ohlc: pd.DataFrame, ticks: pd.DataFrame | None, freq: str) -> pd.DataFrame:
    ...
    price_col = "last" if "last" in df_ticks.columns and df_ticks["last"].notna().any() else "mid"
    ...
    bid_vol = (
        df_ticks["bid_volume"]
        if "bid_volume" in df_ticks.columns
        else df_ticks["volume"]
        if "volume" in df_ticks.columns
        else pd.Series(1.0, index=ts_idx)
    )
    ask_vol = (
        df_ticks["ask_volume"]
        if "ask_volume" in df_ticks.columns
        else df_ticks["volume"]
        if "volume" in df_ticks.columns
        else pd.Series(1.0, index=ts_idx)
    )
    total_vol = bid_vol + ask_vol
    df_ticks["micro_price"] = (df_ticks["bid"] * ask_vol + df_ticks["ask"] * bid_vol) / (total_vol + 1e-10)
    ...
    resampled["tick_intensity"] = tick_count / (pd.Timedelta(freq).total_seconds() / 60.0)
    tick_intensity_1h = resampled["tick_intensity"].rolling(60).mean()
    tick_intensity_std_1h = resampled["tick_intensity"].rolling(60).std()
    resampled["tick_intensity_zscore"] = (resampled["tick_intensity"] - tick_intensity_1h) / (
    ...
    if "close" in ohlc.columns:
        ...
        resampled["spread_efficiency"] = (ohlc["close"] - micro_price_bar) / (spread_bar + 1e-10)
...
def compute_cross_asset_features(
    ...
    for feat_name, other_sym in pairs.items():
        ...
        df[feat_name] = self_returns.rolling(min_obs).corr(other_returns)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Feature engineering from warehouse or tick data")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("Path to warehouse data (parquet dir, single parquet, or CSV). " "Overrides --tick-dir when provided."),
    )
    parser.add_argument(
        "--db-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help=("Comma-separated feature families. " f"Available: {', '.join(FEATURE_FAMILIES)}"),
    )
    ...
    if warehouse_mode:
        ...
        for sym, df in ohlcv_per_sym.items():
                ...
                if col in df.columns:
                    agg_map[col] = (
                        ...
                        if col == "open"
                            ...
                            "max"
                            if col == "high"
                            else ("min" if col == "low" else ("last" if col == "close" else "sum"))
                        )
                    )
            ...
            if "tick_count" in df.columns:
                agg_map["tick_count"] = "sum"
            if "spread_mean" in df.columns:
                agg_map["spread_mean"] = "mean"
            for freq in freqs:
    ...
    for sym in symbols:
        ...
        for freq in freqs:
            ...
            if "microstructure" in feature_families:
                sym_ticks = tick_data.get(sym) if tick_data else None
            ...
            if "cross_asset" in feature_families:
                cross_feats = compute_cross_asset_features(ohlc, by_sym_freq, sym, freq)
