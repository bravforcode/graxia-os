...
def apply_confidence_filter(df: pd.DataFrame, model, feature_cols: list,
...
def combined_filter(df: pd.DataFrame, model, feature_cols: list,
...
def evaluate_filter(df: pd.DataFrame, model, feature_cols: list,
                     regime_threshold: float = 0.55,
    ...
    # Apply filter
    filtered = combined_filter(df_test, model, feature_cols,
                                regime_threshold, min_confidence)
    ...
    for conf in [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80]:
        sig = combined_filter(df_test, model, feature_cols,
                               regime_threshold, conf)['_signal'].values
        tm = sig != 0
        n = tm.sum()
        if n >= 5:
...
def main():
    ...
    parser.add_argument("--mode", choices=["analyze", "filter", "eval"],
                        default="eval", help="Operation mode")
    ...
    parser.add_argument("--typical-move", type=float, default=4.0,
                        help="Typical 1-bar move in pips")
    parser.add_argument("--spread-cost", type=float, default=22.0,
