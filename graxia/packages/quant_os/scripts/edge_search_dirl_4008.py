"""Direction L 4008 — Gold Vol Risk Premium (VRP) backtest."""
...
OUT = ROOT / "reports" / "edge_search_dirl_4008.json"
COST_RT = 0.65  # bps


def main() -> int:
    from graxia.packages.quant_os.strategies.vol_risk_premium import (
    ...
    # Load GVZ from FRED
    gvz = pd.read_csv(ROOT / "data" / "market_data" / "fred" / "GVZCLS.csv")
    gvz["date"] = pd.to_datetime(gvz["date"], utc=True)
    gvz = gvz.set_index(pd.DatetimeIndex(gvz["date"]))["value"].astype(float)
    gvz = gvz.reindex(df.index, method="ffill")

    print(f"XAUUSD D1: {len(df)} bars | GVZ aligned: {int(gvz.notna().sum())}")
    res = compute_vol_risk_premium_signals(
        close=df["close"], implied_vol=gvz, config=VolRiskPremiumConfig()
    )
    ...
    for i in range(1, n):
        ...
        if s != 0 and pos == 0:
            ...
            gross = pos * (prices[i] - entry) * 1000.0  # 1000 oz
