...
def compute_regime_breakdown(df: pd.DataFrame, model, feature_cols: list[str], test_mask=None, baseline_oos=None):
    """
        ...
        for thresh in [2, 3]:
            for vc in vol_cols[:1]:
                ...
                for vol_label, vol_cond in [
                    ...
                    mask = pd.Series((df["_confluence_score"] >= thresh).values & vol_cond.values, index=df.index)
                    evaluate_regime(mask.fillna(False), f"conf>={thresh}+{vc}_{vol_label}")
...
def print_report(results: pd.DataFrame, baseline_acc: float):
    ...
    candidates = results[(results["accuracy"] >= threshold_acc) & (results["pct_bars"] >= 0.05)].sort_values(
    ...
    if not candidates.empty:
        ...
        for _, row in candidates.head(5).iterrows():
            print(
                f"       {row['regime']:<30s} acc={row['accuracy']:.4f}  bars={int(row['n_bars']):>5d} ({row['pct_bars']:.1%})"
            )
    else:
...
def main():
    ...
    parser.add_argument(
        "--model-path", type=str, default=None, help="Path to pre-trained model (optional, trains new if omitted)"
    )
    ...
    results, baseline = compute_regime_breakdown(df, model, feature_cols, test_mask=test_mask, baseline_oos=oos_acc)
    ...
    decision = {
        ...
        "best_accuracy": float(candidates.iloc[0]["accuracy"])
        if candidates is not None and len(candidates) > 0
        else None,
        "best_pct_bars": float(candidates.iloc[0]["pct_bars"])
        if candidates is not None and len(candidates) > 0
        else None,
