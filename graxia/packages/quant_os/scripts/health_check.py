...
def main():
    ...
    check(len(models) > 0, f"ML models: {len(models)}", f"Latest: {models[-1].name[-20:]}" if models else "")
    ...
    try:
        ...
        check(task_count > 0, f"Scheduled tasks: {task_count}", "Data/Sync/Upgrade/Quick/Daily/Research")
    except:
    ...
    # Save
    (BASE / "Meta" / "health_check.json").write_text(
        json.dumps({"timestamp": datetime.now(UTC).isoformat(), **checks}, indent=2)
    )
