...
class BiasDetector:
    ...
    def check_recursive_bias(
        ...
        for key in results[first_count]:
            ...
            if len(last_vals) >= 2 and not all(
                math.isclose(v, last_vals[0], rel_tol=1e-6, abs_tol=1e-9) for v in last_vals
            ):
        ...
        for key in full_result:
            if key in trunc_result:
                ...
                if isinstance(full_val, list) and isinstance(trunc_val, list):
                    ...
                    if not all(
                        math.isclose(a, b, rel_tol=1e-6, abs_tol=1e-9) for a, b in zip(overlap, trunc_val, strict=False)
                    ):
