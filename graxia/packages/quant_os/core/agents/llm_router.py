...
TIER1_CONFIG = ProviderConfig(
    name="cerebras", tier=1, api_key_env="CEREBRAS_API_KEY",
    base_url="https://api.cerebras.ai/v1/chat/completions",
    model="gpt-oss-120b", max_tokens=100, timeout_seconds=3.0,
)

TIER2_CONFIG = ProviderConfig(
    name="groq", tier=2, api_key_env="GROQ_API_KEY",
    base_url="https://api.groq.com/openai/v1/chat/completions",
    model="llama-3.3-70b-versatile", max_tokens=200, timeout_seconds=5.0,
)

TIER3_CONFIG = ProviderConfig(
    name="gemini", tier=3,     api_key_env="GOOGLE_AI_KEY",
    base_url="https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent",
    model="gemini-2.5-flash", max_tokens=500, timeout_seconds=30.0,
)
...
class CascadeRouter:
    ...
    async def route(self, headline: str) -> CascadeResult:
        ...
        if not tier1_response:
            return CascadeResult(headline=headline, impact=ImpactLevel.LOW, direction=0,
                                 tier_used=1, latency_ms=(time.monotonic()-start)*1000,
        ...
        if impact == ImpactLevel.LOW:
            return CascadeResult(headline=headline, impact=impact, direction=direction,
                                 tier_used=1, latency_ms=(time.monotonic()-start)*1000,
        ...
        if not tier2_response:
            return CascadeResult(headline=headline, impact=impact, direction=direction,
                                 tier_used=1, confidence=0.5, reasoning="Tier 2 unavailable",
                                 latency_ms=total)
        tier2_data = self._parse_json(tier2_response)
        if not tier2_data.get("confirmed", True):
            return CascadeResult(headline=headline, impact=ImpactLevel.LOW, direction=0,
                                 tier_used=2, confidence=tier2_data.get("confidence", 0.5),
                                 reasoning=tier2_data.get("reasoning", "rejected"),
                                 latency_ms=total, raw_response=tier2_response)
        return CascadeResult(headline=headline, impact=impact,
                             direction=tier2_data.get("direction", direction),
                             tier_used=2, confidence=tier2_data.get("confidence", 0.7),
