...
class SlippageModel:
    def __init__(self, pip_values: dict[str, float] | None = None):
        self._pip_values = pip_values or {"XAUUSD": 0.01, "EURUSD": 0.0001, "GBPUSD": 0.0001, "USDJPY": 0.01, "BTCUSD": 0.01, "US30": 0.01}
    ...
    def _volatility_multiplier(self, volatility: float | VolatilityRegime | str) -> float:
        if isinstance(volatility, VolatilityRegime):
            return {VolatilityRegime.LOW: 0.5, VolatilityRegime.NORMAL: 1.0, VolatilityRegime.HIGH: 2.0, VolatilityRegime.EXTREME: 4.0}.get(volatility, 1.0)
    ...
    def estimate(self, symbol: str, order_size_lots: float, volatility: float | VolatilityRegime | str = 0.15, session: str | TradingSession = "london") -> SlippageEstimate:
        ...
        return SlippageEstimate(symbol=symbol, base_slippage_pips=base, session_multiplier=session_mult, size_multiplier=size_mult, volatility_multiplier=vol_mult, estimated_slippage_pips=round(estimated, 4), estimated_slippage_price=round(estimated_price, 6), session=session_str, order_size=size_class.value)

    def get_slippage(self, symbol: str, session: str | TradingSession = "london", volatility: float | VolatilityRegime | str = 0.15) -> float:
    ...
    def estimate_cost_per_trade(self, symbol: str, session: str | TradingSession = "london", lots: float = 0.01, volatility: float | VolatilityRegime | str = 0.15, commission_per_lot: float = 0.0) -> dict:
        ...
        return {"spread_pips": est.base_slippage_pips, "slippage_pips": est.estimated_slippage_pips, "total_pips": est.base_slippage_pips + est.estimated_slippage_pips, "total_usd": round(slip_cost + comm_cost, 2)}
