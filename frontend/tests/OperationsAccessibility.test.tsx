...
describe("operations page accessibility baseline", () => {
      ...
      items: [
        {
          ...
          total_score: 8.7,
          fit_summary:
            "High leverage founder program with strong network upside.",
          decision_reasoning:
            "Best blend of speed, brand, and relationship access.",
      ...
      items: [
        {
          ...
          content:
            "Thank you for the opportunity. Here is the sharper version.",
          status: "pending",
          model_used: "gemma-4",
  ...
  it("dashboard has no detectable axe violations with loaded data", async () => {
    const { container } = renderWithQueryClient(<UnifiedDashboard />);

    // UnifiedDashboard uses local mockStats, not API calls
    // Wait for component to render with Thai greeting
    await waitFor(() => {
