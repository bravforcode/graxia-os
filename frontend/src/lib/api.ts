...
export function getRuntimeHealthUrl(apiBaseUrl = API_BASE_URL) {
  ...
  return new URL(
    `${normalizedBaseUrl}${healthPath}`,
    window.location.origin,
  ).toString();
}

function isRuntimeHealthResponse(response: {
  data: unknown;
...
export const api = {
  ...
  getScraperHealth: async (): Promise<{
  ...
  rejectDraft: async (id: string, reason?: string) => {
    const { data } = await client.patch(`/drafts/${id}/reject`, {
      reason: reason ?? "",
    });
    return data;
  },
  ...
  startCheckout: async (plan: string) => {
  ...
  getOnboardingState: async () => {
    const { data } = await client.get("/onboarding/state");
    return data;
  },
  ...
  updateOnboardingProgress: async (
    ...
    const response = await client.post("/onboarding/progress", { step, data });
    return response.data;
  },

  skipOnboarding: async () => {
    const { data } = await client.post("/onboarding/skip");
    return data;
  },

  isOnboardingRequired: async () => {
